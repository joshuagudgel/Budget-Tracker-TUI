...
func TestCategoryValidator(t *testing.T) {
	...
	categories := []string{"Food", "Transportation", "Entertainment", "Utilities"}
	...
	tests := []struct {
		...
		{"Valid category case insensitive", "food", false},
		{"Invalid category", "InvalidCategory", true},
		{"Empty category", "", true},
		{"Whitespace category", "  ", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validator.Validate(tt.category, categories)
			if tt.expectError {
				if err == nil {
					t.Errorf("Expected error but got none")
				}
...
func TestTransactionValidation(t *testing.T) {
	...
	invalidTransaction := &types.Transaction{
		...
		Category:    "InvalidCategory", // Invalid: not in available categories
	}
	...
	t.Run("Invalid transaction", func(t *testing.T) {
		...
		expectedErrors := 4 // amount, description, date, category
		...
		if !result.HasError("category") {
			t.Errorf("Expected category error")
		}
	})
}
